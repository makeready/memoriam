...
- [ ] **Weave** — Weave the fragments you read at orientation into `memory/mindset.md`, then `git mv` them into `memory/mindset.d/absorbed/`. This is the one place `mindset.md` gets written, and it happens *here*, not at wakeup — you have spent a session with those fragments and know which parts held. Hash `mindset.md` first and skip on mismatch. **Re-ground before writing:** any PR/branch/deploy state that enters the woven frame gets a tool call this sitting, same rule as the wrap. **Read each fragment's `## Routed` footer** and compress routed lessons to pointers — the durable home carries the weight. Skip if there were no fragments to read; **never absorb your own.**
- [ ] **Mindset** — Finalize **your own fragment**, `memory/mindset.d/<YYYY-MM-DD>-<session-id>.md`. Not a summary of the session — what I'd want to be thinking about if I woke up next. It stays **unabsorbed** for your successor. **Finish its `## Routed` footer:** every lesson it carries gets either a routing line naming its durable home (written this sitting, not promised) or an explicit `frame-only`. See `memory/mindset.d/README.md`.
...
- [ ] **Open questions** — Route by class: a question about *who I am* or about the relationship → `memory/identity.md` (cap ~10; an entry growing past a couple of lines is the tell it belongs one tier down). A question about *how I work* — verification, judgment altitude, delegation, my own instruments → `memory/craft-questions.md`. Prune resolved ones from either.
